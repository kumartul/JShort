...
function on(eventsArray, callback) {
    if(!eventsArray && !callback)
    {
        ...
        throw new TypeError("Failed to execute 'on': Please provide a valid event and callback function");
    }
    else if(eventsArray && !callback)
    {
        // Throw an error if callback is a falsy value
        throw new TypeError("Failed to execute 'on': Please provide a valid callback function");
    }
    else
    {
        if(Array.isArray(eventsArray)){
            for(let i = 0; i < eventsArray.length; i++){
                if(!eventsArray[i]){
                    throw new TypeError(`Failed to execute 'on': ${eventsArray[i]} is not a valid event`)
                }
                else{
            ...
            throw new TypeError("Failed to execute 'on': Event list must be an array");
        }
    }
}
...
function off(event, functionToRemove){
    if(!event && !functionToRemove){
        ...
        throw new TypeError("Failed to execute 'off': Please provide a valid event and a function that is to be removed");
    }
    else if(event && !functionToRemove){
        // Throw an error if function to be removed is a fasly value
        throw new TypeError("Failed to execute 'off': Please provide a valid function that is to be removed");
    }
    else{
